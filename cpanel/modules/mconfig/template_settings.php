<?php
echo '<h1 class="page-header">Template Settings</h1>';
$templates = getFilesInDirectory(__PATH_TEMPLATES__);
...
echo '<div class="col-md-12">';
	echo '<form action="" method="post">';
		...
		echo '<tr>';
	echo '<td>';
		echo '<strong>Plantilla</strong>';
		echo '<p class="setting-description">Your website\'s default template.</p>';
	echo '</td>';
	echo '<td>';
		echo '<select name="website_template">';
			echo '<option value="' . htmlspecialchars(config('website_template', true)) . '" selected>' . htmlspecialchars(config('website_template', true)) . '</option>';
			foreach ($templates as $template) {
				if ($template !== config('website_template', true)) {
