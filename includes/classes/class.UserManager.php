...
class UserManager {
    ...
    public function getAllUsers(int $limit = 100, int $offset = 0, string $orderBy = _CORE_UREG_, string $orderDir = 'DESC'): array {
        try {
            $orderDir = strtoupper($orderDir) === 'ASC' ? 'ASC' : 'DESC';
            
            $allowedColumns = [_CORE_UID_, _CORE_UNOM_, _CORE_UEML_, _CORE_UREG_, _CORE_LASTLOGIN_, _CORE_USTATUS_];
            $orderBy = in_array($orderBy, $allowedColumns) ? $orderBy : _CORE_UREG_;
            
            $query = "
                SELECT 
                    " . _CORE_UID_ . " as user_id,
                    " . _CORE_UNOM_ . " as username,
                    " . _CORE_UEML_ . " as email,
                    " . _CORE_UREG_ . " as registration_date,
                    " . _CORE_LASTLOGIN_ . " as last_login,
                    " . _CORE_LASTIP_ . " as last_ip,
                    " . _CORE_USTATUS_ . " as status
            ...
            $this->logger?->logException($e, 'USER_GET_ALL');
            return [];
        }
    }
